...
public class JsoupInput extends BaseStep implements StepInterface {
  ...
  public JsoupInput(StepMeta stepMeta, StepDataInterface stepDataInterface, int copyNr,
  ...
  private void handleMissingFiles() throws KettleException {
    ...
    if (nonExistantFiles.size() != 0) {
      ...
      log.logError(BaseMessages.getString(PKG, "JsoupInput.Log.RequiredFilesTitle"), BaseMessages
          .getString(PKG, "JsoupInput.Log.RequiredFiles", message));

      throw new KettleException(BaseMessages.getString(PKG, "JsoupInput.Log.RequiredFilesMissing",
          message));
    }
    ...
    if (nonAccessibleFiles.size() != 0) {
      ...
      log.logError(BaseMessages.getString(PKG, "JsoupInput.Log.RequiredFilesTitle"), BaseMessages
          .getString(PKG, "JsoupInput.Log.RequiredNotAccessibleFiles", message));

      throw new KettleException(BaseMessages.getString(PKG,
          "JsoupInput.Log.RequiredNotAccessibleFilesMissing", message));
    }
  }
        ...
        if (data.indexSourceField < 0) {
          ...
          if (data.indexSourceField < 0) {
            ...
            logError(BaseMessages.getString(PKG,
                "JsoupInput.Log.ErrorFindingField", meta.getFieldValue())); //$NON-NLS-1$ //$NON-NLS-2$
            throw new KettleException(BaseMessages.getString(PKG,
      ...
      if (meta.getIsAFile()) {
        ...
        if (meta.isIgnoreEmptyFile() && data.file.getContent().getSize() == 0) {
          ...
          logBasic(BaseMessages
              .getString(PKG, "JsoupInput.Error.FileSizeZero", data.file.getName()));
          ReadNextString();
        }
      } else {
        // read string
  ...
  private void addFileToResultFilesname(FileObject file) throws Exception {
    if (meta.addResultFile()) {
      ...
      ResultFile resultFile =
          new ResultFile(ResultFile.FILE_TYPE_GENERAL, file, getTransMeta().getName(),
              getStepname());
  ...
  private boolean openNextFile() {
    try {
      ...
      if (meta.isIgnoreEmptyFile() && data.file.getContent().getSize() == 0) {
        // log only basic as a warning (was before logError)
        logBasic(BaseMessages.getString(PKG, "JsoupInput.Error.FileSizeZero", ""
            + data.file.getName()));
        openNextFile();
      }
      readFileOrString();
  ...
  private void parseJsoup() throws Exception {
    ...
    for (int i = 0; i < data.nrInputFields; i++) {
      ...
      if (ja.size() > 0 && (data.nrrecords != -1 && data.nrrecords != ja.size() && ja != null)) {
        throw new KettleException(BaseMessages.getString(PKG, "JsoupInput.Error.BadStructure", ja
            .size(), path, prevPath, data.nrrecords));
      }
      resultList.add(ja);
  ...
  public boolean processRow(StepMetaInterface smi, StepDataInterface sdi) throws KettleException {
    ...
    try {
      ...
      if (log.isRowLevel())
        logRowlevel(BaseMessages.getString(PKG, "JsoupInput.Log.ReadRow", data.outputRowMeta
            .getString(r)));
      incrementLinesInput();
      data.rownr++;
  ...
  private Object[] buildRow() throws KettleException {
    ...
    data.previousRow = irow == null ? outputRowData : (Object[]) irow.cloneRow(outputRowData); // copy
    // it
    // to
    // make
